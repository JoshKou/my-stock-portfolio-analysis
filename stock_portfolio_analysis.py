...
def calc_returns(portfolio):
    ...
    portfolio['Stock Return'] = (portfolio['Ticker Share Value'] / portfolio['Adj cost'] - 1) * 10
    portfolio['Benchmark Return New'] = (portfolio['Benchmark Share Value'] / portfolio['Adj cost'] - 1) * 10 
...
stocks_start = datetime.datetime(2021, 4, 11)
stocks_end = datetime.datetime(2021, 7, 15)

daily_adj_close = get_data(symbols, stocks_start, stocks_end)
daily_adj_close = daily_adj_close[['Close']].reset_index()
daily_benchmark = get_benchmark(['^GSPC'], stocks_start, stocks_end)
...
# Percentage portfolio vs benchmark returns over time VERY INNACURATE
# line(combined_df, 'Stock Return', 'Benchmark Return New')

# Nominal portfolio vs benchmark returns over time
line(combined_df, 'Ticker Share Value', 'Benchmark Share Value') 
